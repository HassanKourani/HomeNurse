...
CREATE TABLE profiles (
    ...
    email TEXT,
    role TEXT NOT NULL CHECK (role IN ('registered', 'physiotherapist', 'nurse', 'patient', 'superAdmin')),
    full_name TEXT,
...
Each role has specific permissions and access levels within the system. Healthcare providers must be approved by administrators before they can accept requests.
...
3. **delete_nurse_request_hours**

```sql
-- Deletes all working hours logs for a nurse on a specific request (superAdmin only)
FUNCTION delete_nurse_request_hours(rid integer, nid uuid) RETURNS jsonb
```

This function:

- Deletes all working hours logs for a specific nurse on a specific request
...
- Security:
  ...
  - Includes role validation checks
